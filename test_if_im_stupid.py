...
if __name__ == "__main__":
    ...
    env = BeamManagementEnv(num_antennas = 16,
    ...
    for epi in range(num_epoch):        
        ...
        while not done:    
            ...
            exhaustive_epi += info['exhaustive_reward']
            test_epi += r
            epi_step += 1
    ...
    plt.figure()
    sns.kdeplot(test,label='agent')
    sns.kdeplot(baseline,label='baseline')
    sns.kdeplot(genie,label='genie')
    sns.kdeplot(exhaustive,label='exhaustive')
    plt.legend();    
